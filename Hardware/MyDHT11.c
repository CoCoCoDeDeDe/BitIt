...
uint32_t MyDHT11_ReadIntervalCount;//存储FALLING和RISING之间TIM3CNT计数次数
uint32_t MyDHT11_ReadIntervalCount_Start;//存储开始记FALLING和RISING之间TIM3CNT计数次数时的TIM3CNT值
uint8_t MyDHT11_Flag_ReadSucced = 0;

void MyDHT11_Init(void) {
	
	//GPIO_InitTypeDef GPIO_InitStruct;全局变量
	...
	//EXTI_InitTypeDef EXTI_InitStruct;全局变量
...
// 决定是否记录TIM3的CNT重装次数,开始读取数据前默认不记录
...
void MyDHT11_WriterSM(void) {
	static uint8_t RunTimesCounter;
	if(MyDHT11_Writer_Ctrl  == 1) {//当开启写入复位信号时才运行函数主要内容
		RunTimesCounter ++;//只要进该函数就记录1次,无论状态几
		switch(MyDHT11_WriterSM_State) {
			case 0:
				MyDHT11_SetWriteMode();//设PA4为输出模式
				MyDHT11_WriterRESET();//拉低电平
				MyDHT11_WriterSM_State = 1;//下次来本函数进状态1
				RunTimesCounter = 0;//重新计时,记录低电平持续时间
				Serial_SendStringPacket(USART2, "WriterSM_State: 0\r\n");
				break;
			case 1://持续20ms低电平
				if(RunTimesCounter * 5 >= 20000) {//持续20ms低电平
					MyDHT11_WriterSET();//拉高电平
					MyDHT11_WriterSM_State = 2;//下次来本函数进状态2
					RunTimesCounter = 0;//重新计时,记录高电平持续时间
				Serial_SendStringPacket(USART2, "WriterSM_State: 1 in\r\n");
				}
				//Serial_SendStringPacket(USART2, "WriterSM_State: 1 out\r\n");
				break;
			case 2://持续30us高电平然后准备下回合写入以及本回合读取
				if(RunTimesCounter * 5 >= 30) {//持续30us高电平
					...
					RunTimesCounter = 0;//重新计时,用于下回合记录高电平持续时间
					MyDHT11_WriterSM_State = 0;//下次来本函数进状态0
					MyDHT11_Writer_Ctrl = 0;//关闭Writer
				}
				Serial_SendStringPacket(USART2, "WriterSM_State: 2\r\n");
				break;
...
void MyDHT11_ReaderSM(void) {
	...
	static uint8_t MyDHT11_ReadBitCount;//在成功接收1Bit时++,在接收完40BIT或者开始接收时=0
	...
	switch (MyDHT11_ReaderSM_State) {
		...
		case BIT_FALLING:
			...
			if(MyDHT11_ReadIntervalCount <25 || 29 < MyDHT11_ReadIntervalCount ||
		MyDHT11_ReadIntervalCount < 68 || 73 < MyDHT11_ReadIntervalCount) {
				MyDHT11_ReadFailed();//失败
			} else {//成功
				MyDHT11_ReadBitCount ++;//记录接收BIT的次数
				MyDHT11_StartReadIntervalCount();//准备接收新Count
				if(25 <= MyDHT11_ReadIntervalCount || MyDHT11_ReadIntervalCount <= 29) {
					MyDHT11_BitsArr[MyDHT11_ReadBitCount - 1] = 0;//第MyDHT11_ReadBitCount个Bit收的是0,存到数组第MyDHT11_ReadBitCount个位置
				} else if(68 <= MyDHT11_ReadIntervalCount || MyDHT11_ReadIntervalCount <= 73) {
...
void MyDHT11_StartReadIntervalCount(void) {
	...
	MyDHT11_TIM3ARCount = 0; //重置TIM3的AR次数,只记本Interval内的AR次数
}
